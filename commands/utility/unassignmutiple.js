...
module.exports = {
  ...
  async execute(interaction, client, config) {
    ...
    if (invalidRoles.length > 0) {
      const errorEmbed = new EmbedBuilder()
        .setDescription(`❌ You cannot remove roles equal or higher than your highest role: ${invalidRoles.map(r => r.name).join(', ')}`)
        .setColor('#FF0000');
    ...
    if (successfulRoles.length > 0) {
      description += `❌ Removed roles: ${successfulRoles.map(r => `${r}`).join(' ')} from ${user}.\n`;
    }
